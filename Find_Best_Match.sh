...
all_subtitles_dir="$all_subtitles_dir/modified"
...
find "$all_subtitles_dir/MakeMKV" -name "*.txt" -print0 | while read -d $'\0' dvdsub
do
    number_of_lines=$( cat "$dvdsub" | wc -l)
    find "$all_subtitles_dir/OST" -name "*.txt" -print0 | while read -d $'\0' file
    do
